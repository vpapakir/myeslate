...
/**
 * Cell renderer for the button group editor of the E-Slate toolbar.
 *
 * @author      Kriton Kyrimis
 * @version     2.0.0, 22-May-2006
 */
public class ButtonGroupCellRenderer implements TreeCellRenderer
{
  /**
   * The label with the name of the node corresponding to the tool or button
   * group node that is currently being displayed.
   */
  private JLabel textLabel;
  /**
   * The label with the icon of the node corresponding to the tool or button
   * group node that is currently being displayed.
   */
  ...
  ButtonGroupCellRenderer()
  {
    ...
    rootLabel.setText(ButtonGroupEditor.resources.getString("root"));
  ...
  {
    ButtonGroupNode node = (ButtonGroupNode)value;
    Component comp;
    JLabel lab;
    if (node.isRoot()) {
      comp = rootLabel;
   ...
   */
  Font getFont(ButtonGroupNode node)
  {
    if (node.isRoot()) {
   ...
   */
  ImageIcon getIcon(ButtonGroupNode node)
  {
   ...
   *            given node.
   */
  int getIconTextGap(ButtonGroupNode node)
  {
    if (node.isRoot()) {
      return 0;
    }else{
   ...
   *            node.
   */
  Dimension getPreferredSize(ButtonGroupNode node)
  {
    Component c =
