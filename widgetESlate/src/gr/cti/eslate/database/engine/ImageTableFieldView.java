...
/** ImageTableFieldView implements a view on a ImageTableField, called the base field. A view of a table field is
 ...
 */
// The 'data' IntBaseArray of the ImageTableFieldView is always empty.
public class ImageTableFieldView extends ImageTableField implements TableFieldView {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The field on which this view is based. */
    ImageTableField baseField = null;
    ...
    public ImageTableFieldView() {
    }

    protected ImageTableFieldView(ImageTableField field) {
        super(field.getName(), field.isEditable(), field.isRemovable(), field.isHidden());
        baseField = field;
    }

    /** Sets the TableView of the ImageTableFieldView */
    void setTable(Table table) {
        ...
        if (baseField == null && baseFieldName != null) {
            try{
                baseField = (ImageTableField) tableView.getBaseTable().getTableField(baseFieldName);
            }catch (InvalidFieldNameException exc) {
                System.out.println("InvalidFieldNameException in ImageTableFieldView setTable(). Should never occur...");
            }
            baseFieldName = null;
        }
    }
...
//        baseField.remove(recIndex);
    ...
    public ArrayBase getData() {
        int dataSize = size();
        CImageIconBaseArray array = new CImageIconBaseArray(dataSize);
    ...
    public Object[] getDataArray() {
        CImageIcon[] ar = new CImageIcon[size()];
    ...
    public IntBaseArray getOrder(ObjectComparator comparator) {
        ...
        return getOrder(data, 0, data.size()-1, (ImageComparator) comparator);
    }

    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        ObjectBaseArray data = getDataBaseArray();
        return getOrder(data, first, last, (ImageComparator) comparator);
    }
