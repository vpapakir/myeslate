...
/** DateTableFieldView implements a view on a DateTableField, called the base field. A view of a table field is
 ...
 */
// The 'data' IntBaseArray of the DateTableFieldView is always empty.
public class DateTableFieldView extends DateTableField implements TableFieldView {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The field on which this view is based. */
    DateTableField baseField = null;
    ...
    public DateTableFieldView() {
    }

    protected DateTableFieldView(DateTableField field) {
        super(field.getName(), field.isEditable(), field.isRemovable(), field.isHidden());
        baseField = field;
    }

    /** Sets the TableView of the DateTableFieldView */
    void setTable(Table table) {
        ...
        if (baseField == null && baseFieldName != null) {
            try{
                baseField = (DateTableField) tableView.getBaseTable().getTableField(baseFieldName);
            }catch (InvalidFieldNameException exc) {
                System.out.println("InvalidFieldNameException in DateTableFieldView setTable(). Should never occur...");
            }
            baseFieldName = null;
        }
    }
...
//        baseField.remove(recIndex);
    ...
    public ArrayBase getData() {
        int dataSize = size();
        CDateBaseArray array = new CDateBaseArray(dataSize);
    ...
    public IntBaseArray getOrder(ObjectComparator comparator) {
        ...
        return getOrder(data, 0, data.size()-1, (DateComparator) comparator);
    }

    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        ObjectBaseArray data = getDataBaseArray();
        return getOrder(data, first, last, (DateComparator) comparator);
    }
