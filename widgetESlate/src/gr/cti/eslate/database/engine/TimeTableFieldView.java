...
/** TimeTableFieldView implements a view on a TimeTableField, called the base field. A view of a table field is
 ...
 */
// The 'data' IntBaseArray of the TimeTableFieldView is always empty.
public class TimeTableFieldView extends TimeTableField implements TableFieldView {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The field on which this view is based. */
    TimeTableField baseField = null;
    ...
    public TimeTableFieldView() {
    }

    protected TimeTableFieldView(TimeTableField field) {
        super(field.getName(), field.isEditable(), field.isRemovable(), field.isHidden());
        baseField = field;
    }

    /** Sets the TableView of the TimeTableFieldView */
    void setTable(Table table) {
        ...
        if (baseField == null && baseFieldName != null) {
            try{
                baseField = (TimeTableField) tableView.getBaseTable().getTableField(baseFieldName);
            }catch (InvalidFieldNameException exc) {
                System.out.println("InvalidFieldNameException in TimeTableFieldView setTable(). Should never occur...");
            }
            baseFieldName = null;
        }
    }
...
//        baseField.remove(recIndex);
    ...
    public ArrayBase getData() {
        int dataSize = size();
        CTimeBaseArray array = new CTimeBaseArray(dataSize);
    ...
    public IntBaseArray getOrder(ObjectComparator comparator) {
        ...
        return getOrder(data, 0, data.size()-1, (TimeComparator) comparator);
    }

    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        ObjectBaseArray data = getDataBaseArray();
        return getOrder(data, first, last, (TimeComparator) comparator);
    }
