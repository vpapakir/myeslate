...
/** DoubleTableFieldView implements a view on a DoubleTableField, called the base field. A view of a table field is
 ...
 */
// The 'data' IntBaseArray of the DoubleTableFieldView is always empty.
public class DoubleTableFieldView extends DoubleTableField implements TableFieldView {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The field on which this view is based. */
    DoubleTableField baseField = null;
    ...
    public DoubleTableFieldView() {
    }

    protected DoubleTableFieldView(DoubleTableField field) {
        super(field.getName(), field.isEditable(), field.isRemovable(), field.isHidden());
        baseField = field;
    }

    /** Sets the TableView of the DoubleTableFieldView */
    void setTable(Table table) {
        ...
        if (baseField == null && baseFieldName != null) {
            try{
                baseField = (DoubleTableField) tableView.getBaseTable().getTableField(baseFieldName);
            }catch (InvalidFieldNameException exc) {
                System.out.println("InvalidFieldNameException in DoubleTableFieldView setTable(). Should never occur...");
            }
            baseFieldName = null;
        }
    }
...
//System.out.println(fieldName + " recIndex: " + recIndex + ", tableView.records.get(recIndex): " + tableView.records.get(recIndex));
//System.out.println("recIndex: " + recIndex + ", tableView.records.size(): " + tableView.records.size() + ", recordCount: " + tableView.getRecordCount() + ", value: " + baseField.getObjectAt(tableView.records.get(recIndex)));
////System.out.println("getObjectAt() " + getName() + ", rec: " +recIndex + ": "  + baseField.getObjectAt(tableView.records.get(recIndex)));
...
//        baseField.remove(recIndex);
    ...
    public ArrayBase getData() {
        int dataSize = size();
        DblBaseArray array = new DblBaseArray(dataSize);
...
*/
    ...
    public IntBaseArray getOrder(ObjectComparator comparator) {
        DblBaseArray data = (DblBaseArray) getData();
        return getOrder(data, 0, data.size()-1, (DoubleComparator) comparator);
    }

    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        DblBaseArray data = (DblBaseArray) getData();
        return getOrder(data, first, last, (DoubleComparator) comparator);
    }
     ...
     * @see DoubleTableField#getCurrency()
     */
    ...
    /**
     * Dispatches the method call to the base field.
     * @see DoubleTableField#setCurrency(int)
     */
