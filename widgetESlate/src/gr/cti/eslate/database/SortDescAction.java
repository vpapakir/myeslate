...
public class SortDescAction extends AbstractAction {
    ...
    public SortDescAction(Database db, String name){
        iconEnabled = new ImageIcon(getClass().getResource("images/toolbar/sortDesc.gif"));
        iconDisabled = new ImageIcon(getClass().getResource("images/toolbar/sortDescDisabled.gif"));
        dBase = db;
        actionName = name;
    }

    public SortDescAction(Database db, String name, Icon iconEnabled, Icon iconDisabled) {
        dBase = db;
...
//1                  String fldName = (String) dBase.activeDBTable.tableModel.getColumnName(((Integer) columnOrder.at(fieldIndices[0])).intValue());
                  dBase.activeDBTable.sortOnField(fldName, false, 0, dBase.activeDBTable.table.getRecordCount()-1, true);
...
//                      fieldNames.add((String) dBase.activeDBTable.tableModel.getColumnName(((Integer) columnOrder.at(fieldIndices[i])).intValue()));
    ...
    public void setEnabled(boolean b) {
        super.setEnabled(b);
        dBase.standardToolbarController.setSortDescEnabled(b);
        dBase.menu.miSortDescending.setEnabled(b);
    }
}
