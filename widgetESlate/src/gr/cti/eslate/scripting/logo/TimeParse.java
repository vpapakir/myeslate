...
public class TimeParse {
     ...
     */
    static TimeCount parseTime(LogoObject obj) {
        TimeCount tc;

        if (obj instanceof LogoWord) {
            tc = parseTime(((LogoWord) obj).toString());
        } else {
            if (obj instanceof LogoList) {
                tc = parseTime((LogoList) obj);
            } else {
                tc = null;
            }
        }
     ...
     * @param	return	The parsed time or null if the time cannot be parsed.
     */
    private static TimeCount parseTime(String s) {
     ...
     * @param	return	The parsed time or null if the time cannot be parsed.
     */
    private static TimeCount parseTime(LogoList l) {
        int length = l.length();
        ...
        try {
            ...
            if (length > 2) {
                //      System.out.println("parsed : "+ l.pickInPlace(2).toInteger() + " seconds");
