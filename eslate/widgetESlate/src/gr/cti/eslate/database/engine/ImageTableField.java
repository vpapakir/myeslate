...
public class ImageTableField extends AbstractTableField implements Externalizable {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The data type of the data of the ImageTableField. */
    public static final Class DATA_TYPE = CImageIcon.class;
    private CImageIconBaseArray data = new CImageIconBaseArray();
     ...
     */
    private CImageIcon newCellValue = null;

    static {
        queryComparators.put("<", new LessHashComparator());
        queryComparators.put("<=", new LessEqualHashComparator());
        queryComparators.put(">", new GreaterHashComparator());
        queryComparators.put(">=", new GreaterEqualHashComparator());
        queryComparators.put("!=", new NotEqualHashComparator());
        queryComparators.put("=", new EqualHashComparator());
    }

	public ImageTableField() {
	}

	protected ImageTableField(String name, boolean editable, boolean removable, boolean hidden) {
		fieldName = name;
		isEditable = editable;
...
//            data.add(null);
    ...
    /** Returns the class <code>gr.cti.eslate.database.engine.CImageIcon</code>. */
	public final Class getDataType() {
		return DATA_TYPE;
	}

    /** Returns the localized data type name of the ImageTableField. */
    public String getLocalizedDataTypeName() {
        return DBase.resources.getString("Image");
    }
     ...
     *  @return The value of the cell as a CImageIcon.
     *  @exception InvalidCellAddressException Invalid cell address.
     */
    public CImageIcon getCell(int recIndex)  throws InvalidCellAddressException {
    ...
    public String getCellAsString(int recIndex) {
        CImageIcon value = getValueAt(recIndex);
    ...
    /** Returns the value of this field at <code>recIndex</code> as a CImageIcon.*/
    CImageIcon getValueAt(int recIndex) {
        return get(recIndex);
    }
    ...
    /**
     *  Sets the specified cell of the ImageTableField to <code>value</code>.
     *  @param recIndex The index of the record which contains the cell.
    ...
    InvalidDataFormatException, NullTableKeyException, DuplicateKeyException {
        // Convert the Object 'value' to CImageIcon.
        if (value != null && value instanceof String && ((String) value).trim().length() == 0)
            value = null;

        CImageIcon newValue = null;
        if (value != null) {
            if (!getDataType().isInstance(value)) {
                newValue = (CImageIcon) table.convertToFieldType(value, this);
            }else
                newValue = (CImageIcon) value;
        }
        /* The "referenceToExternalFile" attribute of the CImageIcon has to be synchronized against the
         * field's "linkToExternalData" attribute.
         */
        newValue.referenceToExternalFile = this.linkToExternalData;
        ...
        CImageIcon currentValue = get(recIndex);
    ...
    void set(int recIndex, Object value) {
        set(recIndex, (CImageIcon) value);
    }

    /** Directly sets the value of the cell at <code>recIndex</code> of this field. */
    void set(int recIndex, CImageIcon value) {
        data.set(recIndex, value);
    }

    /** Returns the value of the cell at <code>recIndex</code>. */
    CImageIcon get(int recIndex) {
        return data.get(recIndex);
    }
    void add(Object value) {
        data.add((CImageIcon) value);
    }
    ...
    public int indexOf(int start, int end, Object value) {
        if (value != null && !CImageIcon.class.isAssignableFrom(value.getClass()))
            return -1;
        CImageIcon im = (CImageIcon) value;
        return data.indexOf(start, end, im);
    }
    ...
    public void findNext(FieldDataIterator iterator, SecondPredicateComparator predicateComparator) {
        while (!iterator.atEnd() && (!((ImageSecondPredicateComparator) predicateComparator).execute(((ImageFieldDataIterator) iterator).get())))
          iterator.advance();
    }
...
/*    public FieldDataIterator findNext(FieldDataIterator first, FieldDataIterator last, SecondPredicateComparator predicateComparator) {
        ImageFieldDataIterator firstx = (ImageFieldDataIterator) first.clone();
        while (!firstx.equals(last) && (*//*firstx.get() == null || *//* !((ImageSecondPredicateComparator) predicateComparator).execute(firstx.get())))
          firstx.advance();
...
*/
     ...
     *  Currently no comparators are supported for ImageTableFields.
     */
    public static final ImageComparator getComparator(String comparatorStr) {
        return (ImageComparator) queryComparators.get(comparatorStr);
    }
    ...
    /** Returns a copy of the comparators supported by the ImageTableField.
     */
     ...
     *  @return An instance of ImageFieldDataIterator.
    ...
    public FieldDataIterator iterator(int firstIndex) {
        ...
        for (int i=0; i<dataSize; i++)
            recIndices.add(i);
        return new ImageFieldDataIterator(recIndices, firstIndex, this);
    }
     ...
     *  @see #getComparator(String)
     */
    public boolean compareTo(int recIndex, ImageComparator comparator, CImageIcon value) {
    ...
    /** @see #compareTo(int, ImageComparator, CImageIcon)
     */
    public boolean compareTo(int recIndex, ObjectComparator comparator, Object value) {
        return compareTo(recIndex, (ImageComparator) comparator, (CImageIcon) value);
    }
     ...
     *  @see #sort(ObjectBaseArray, int, int, ImageComparator)
     */
    public void sort(ObjectBaseArray data, ImageComparator comparator) {
     ...
     *  @param comparator The Comparator used to sort the array.
     */
    public void sort(ObjectBaseArray data, int first, int last, ImageComparator comparator) {
...
//        int finish = (base.start()).distance( last );
     ...
     *  @return The new order of the elements of the array. For example the first int in this array contains the index
     *  that the first element in the sorted <code>data</code> array had before sorting.
     *  @see #getOrder(ObjectBaseArray, int, int, ImageComparator)
     */
    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        ...
        for (int i=0; i<dataSize; i++)
            dataClone.add(get(i));
        return getOrder(dataClone, first, last, (ImageComparator) comparator);
    }
     ...
     *  that the first element in the sorted <code>data</code> array had before sorting.
     */
    public static IntBaseArray getOrder(ObjectBaseArray data, int first, int last, ImageComparator comparator) {
...
//        int finish = (base.start()).distance( last );
    ...
    void setNewCellValue(Object value) {
        newCellValue = (CImageIcon) value;
        if (newCellValue != null)
            newCellValue.referenceToExternalFile = containsLinksToExternalData();
    }
    ...
    public void applyState(StorageStructure fieldMap) {
        ...
        data = (CImageIconBaseArray) fieldMap.get("Data");
    }
...
class ImageSecondPredicateComparator implements SecondPredicateComparator {
    CImageIcon secondPredicate = null;
    ImageComparator comparator = null;

    public ImageSecondPredicateComparator(ImageComparator comparator, CImageIcon secondPredicate) {
        this.secondPredicate = secondPredicate;
        this.comparator = comparator;
    }

    public boolean execute(CImageIcon firstPredicate) {
...
abstract class ImageComparator implements ObjectComparator {
    public abstract boolean execute(CImageIcon first, CImageIcon second);
    public boolean execute(Object first, Object second) {
        return execute((CImageIcon) first, (CImageIcon) second);
    }
}
