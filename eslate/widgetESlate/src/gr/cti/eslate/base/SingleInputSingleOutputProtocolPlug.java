...
/**
 * Implements single input, single output mixed mode plugs.
 *
 * @author      Kriton Kyrimis
 * @version     2.0.21, 28-Sep-2007
 */
public class SingleInputSingleOutputProtocolPlug extends MixedModePlug
{
  /**
   * Constructs a single input, single output mixed mode plug.
  ...
  public SingleInputSingleOutputProtocolPlug(ESlateHandle handle,
  ...
  public ImageIcon getIcon()
  {
    ...
    if (providers > 0 && dependents > 0) {
      if (icon_io == null) {
        icon_io = createPlugIcon(Plug.class.getResource("siso.gif"));
      }
      return icon_io;
    } else {
      if (providers > 0) {
        if (icon_i == null) {
          icon_i = createPlugIcon(Plug.class.getResource("siso_i.gif"));
        }
        return icon_i;
      } else {
        if (dependents > 0) {
          if (icon_o == null) {
            icon_o = createPlugIcon(Plug.class.getResource("siso_o.gif"));
          }
          return icon_o;
        } else {
          if (icon == null) {
            icon = createPlugIcon(Plug.class.getResource("siso.gif"));
          }
          return icon;
        }
      }
