 ...
 *
 * Printing code adapted from http://www.cafeaulait.org/course/week13/07.html
 * for which the following Copyright notice holds:
 * <P>
 ...
 *
 * @author Slava Pestov
 * @version $Id: JEditTextArea.java,v 1.2 2008/05/08 10:00:35 kyrimis Exp $
 */
public class JEditTextArea extends JComponent
{
    ...
    /**
     * Do not replace strings after displaying the "replace" dialog.
     */
    private boolean onlyFind = false;
	/**
	...
	/**
	 * Ensures that the caret is visible by scrolling the text area
         * horizontally, if necessary. The text area is not scrolled
         * vertically.
	 * @return True if scrolling was actually performed, false if the
	 * caret was already visible
	 */
	public boolean scrollToCaretPreserveCaretLine()
	{
		int line = getCaretLine();
		int lineStart = getLineStartOffset(line);
		int offset = Math.max(0,Math.min(getLineLength(line) - 1,
	...
	/**
	 * Removes text from the text area.
	...
	public void select(int start, int end)
	{
		...
		magicCaret = -1;
                ...
                int line = getCaretLine();
                int lastLine = firstLine + getVisibleLines() - 1;
                if (firstLine > line || lastLine < line ) {
                  scrollToCaret();
                }else{
                  scrollToCaretPreserveCaretLine();
                }
	}
	...
	public void paste()
	{
        ...
        public void find()
        {
          ...
          fd.setCaseSensitive(caseSensitive);
          fd.setBackwardsSearch(backwards);
          if (findText != null) {
          ...
          if (fd.findText != null) {
            ...
            backwards = fd.isBackwards;
            caseSensitive = fd.isCaseSensitive;
            replaceText = null;
            findAndReplace(findText, null);
          }
        }
        ...
        public void replace()
        {
          ...
          rd.setCaseSensitive(caseSensitive);
          rd.setBackwardsSearch(backwards);
          if (findText != null) {
            rd.setFindText(findText);
          }
          if (findText != null) {
            rd.setReplaceText(replaceText);
          }
          rd.setVisible(true);
          if (rd.findText != null) {
            onlyFind = rd.onlyFind;
            findText = rd.findText;
            replaceText = rd.replaceText;
            backwards = rd.isBackwards;
            caseSensitive = rd.isCaseSensitive;
            if (rd.replaceAll) {
        ...
        /**
         * Find the next occurrence of a string, optionally replacing it with
         * a new string.
         * @param       text    The string.
         * @param       newText The replacement string. If <code>null</code>,
         *                      no replacement will take place.
         */
        public void findAndReplace(String text, String newText)
        {
          ...
          if (caseSensitive) {
            ...
            fullText = StringUtils.toUpperCase(getText());
          ...
          if (backwards) {
            ...
            if (current >= length) {
              if (fullText.substring(current-length, current).equals(text)) {
                fullText = fullText.substring(0, current-length);
              }
            }else{
              return;
            }
            newPos = fullText.lastIndexOf(text, current);
            if (newPos >= 0) {
              select(newPos, newPos+text.length());
              if (!onlyFind && newText != null) {
                setSelectedText(newText);
                select(newPos, newPos+newText.length());
              }
            }
          }else{
            newPos = fullText.indexOf(text, current);
            if (newPos >= 0) {
              select(newPos, newPos+text.length());
              if (!onlyFind && newText != null) {
                setSelectedText(newText);
         ...
         */
        public void replaceAll(String text, String newText)
        {
          try {
          int pos = getLineStartOffset(getLineOfOffset(getCaretPosition()));
          String fullText = getText();
          if (!caseSensitive) {
            fullText = StringUtils.toUpperCase(fullText);
          }
          ...
          for (int i=results.size()-1; i>=0; i--) {
            loc = results.get(i);
            fullText = fullText.substring(0, loc) +
                       newText +
                       fullText.substring(loc + length);
          }
        ...
        {
          GotoLineDialog gd = new GotoLineDialog(this, getLineCount());
          gd.setVisible(true);
          if (gd.line != null) {
            gotoLine(gd.line);
          }
        ...
        public void gotoLine(int line)
        {
          try {
            ...
            // Don't go to non-existent lines.
          }
        }
        ...
        public void print()
        {
          ...
          if (pjob != null) {
            ...
            if (pg != null) {
              printLongString(pjob, pg, getText());
              pg.dispose();
            }
            pjob.end();
          }
        ...
        // Deal with word wrap by chopping long lines into smaller pieces,
        // and with tabs by expanding them into spaces.
        private void printLongString (PrintJob pjob, Graphics pg, String s)
        {
          // We're assuming a 2 cm margin on all
          // four sides. This could be done better.
          int margin = (int)Math.round(72.0 * 2.0 / 2.54);

          // Maximum line length. Longer lines are wrapped.
          int maxLineSize = 80;
          ...
          try {
            do {
              if (pendingLine == null) {
                pendingLine = expandTabs(lnr.readLine());
              }
              if (pendingLine != null) {
                if (pendingLine.length() > maxLineSize) {
                  nextLine = pendingLine.substring(0, maxLineSize);
                  pendingLine = pendingLine.substring(maxLineSize);
                }else{
                  nextLine = pendingLine;
                  pendingLine = null;
                }
              }else{
                nextLine = null;
              ...
              if (nextLine != null) {         
                if ((curHeight + fontHeight) > pageHeight) {
                  // New Page
                  //System.out.println(
                  //  linesForThisPage + " lines printed for page " + pageNum
                  //);
        ...
        private String expandTabs(String s)
        {
          ...
          int tabSize =
            (Integer)getDocument().getProperty(PlainDocument.tabSizeAttribute);
          int pos = 0;
          int nChars = s.length();
	...
	class ScrollLayout implements LayoutManager
	{
		...
		public void layoutContainer(Container parent)
		{
			...
			Iterator<Component> status = leftOfScrollBar.iterator();
			while(status.hasNext())
			{
				Component comp = status.next();
