...
public class TimeTableField extends AbstractTableField implements Externalizable {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The data type of the data of the TimeTableField. */
    public static final Class DATA_TYPE = CTime.class;
    private CTimeBaseArray data = new CTimeBaseArray();
     ...
     */
    private CTime newCellValue = null;

    static{
        queryComparators.put("=", new EqualTime());
        queryComparators.put("!=", new NotEqualTime());
        queryComparators.put("<", new LessTime());
        queryComparators.put("<=", new LessEqualTime());
        queryComparators.put(">", new GreaterTime());
        queryComparators.put(">=", new GreaterEqualTime());
    }

	public TimeTableField() {
	}

	protected TimeTableField(String name, boolean editable, boolean removable, boolean hidden) {
		fieldName = name;
		isEditable = editable;
...
//            data.add(null);
    ...
    /** Returns the class <code>gr.cti.eslate.database.engine.CTime</code>. */
	public final Class getDataType() {
		return DATA_TYPE;
	}

    /** Returns the localized data type name of the TimeTableField. */
    public String getLocalizedDataTypeName() {
        return DBase.resources.getString("Time");
    }

    /**
     *  @return Returns true, if <code>value</code> is null. <code>false</code> otherwise.
     */
    public static final boolean isNull(CTime value) {
        return (value == null);
    }
     ...
     *  @return The value of the cell as a CTime object.
     *  @exception InvalidCellAddressException Invalid cell address.
     */
    public CTime getCell(int recIndex) throws InvalidCellAddressException {
    ...
    public String getCellAsString(int recIndex) {
        CTime value = getValueAt(recIndex);
    ...
    /** Returns the value of this field at <code>recIndex</code> as a CTime object. */
    CTime getValueAt(int recIndex) {
        return get(recIndex);
    }
    ...
    /**
     *  Sets the specified cell of the TimeTableField to <code>value</code>.
     *  @param recIndex The index of the record which contains the cell.
    ...
    InvalidDataFormatException, NullTableKeyException, DuplicateKeyException {
        // Convert the Object 'value' to CTime.
        ...
        if (value != null) {
            if (!getDataType().isInstance(value)) {
                newValue = (CTime) table.convertToFieldType(value, this);
            }else
                newValue = (CTime) value;
        }
        ...
        CTime currentValue = get(recIndex);
    ...
    public int indexOf(int start, int end, Object value) {
        if (value != null && !CTime.class.isAssignableFrom(value.getClass()))
            return -1;
        CTime t = (CTime) value;
        return data.indexOf(start, end, t);
    }
    ...
    public void findNext(FieldDataIterator iterator, SecondPredicateComparator predicateComparator) {
        while (!iterator.atEnd() && (!((TimeSecondPredicateComparator) predicateComparator).execute(((TimeFieldDataIterator) iterator).get())))
          iterator.advance();
    }
...
/*    public FieldDataIterator findNext(FieldDataIterator first, FieldDataIterator last, SecondPredicateComparator predicateComparator) {
        TimeFieldDataIterator firstx = (TimeFieldDataIterator) first.clone();
        while (!firstx.equals(last) && (*//*firstx.get() == null || *//* !((TimeSecondPredicateComparator) predicateComparator).execute(firstx.get())))
          firstx.advance();
...
*/
     ...
     *  The comparators supported for TimeTableFields are:
        <table border="1" width="100%">
          ...
          <tr>
            ...
            <td width="50%" align="center">EqualTime</td>
          </tr>
          <tr>
            <td width="50%" align="center">!=</td>
            <td width="50%" align="center">NotEqualTime</td>
          </tr>
          <tr>
            <td width="50%" align="center">&lt;</td>
            <td width="50%" align="center">LessTime</td>
          </tr>
          <tr>
            <td width="50%" align="center">&lt;=</td>
            <td width="50%" align="center">LessEqualTime</td>
          </tr>
          <tr>
            <td width="50%" align="center">></td>
            <td width="50%" align="center">GreaterTime</td>
          </tr>
          <tr>
            <td width="50%" align="center">>=</td>
            <td width="50%" align="center">GreaterEqualTime</td>
          </tr>
        </table>
     */
    public static final TimeComparator getComparator(String comparatorStr) {
        return (TimeComparator) queryComparators.get(comparatorStr);
    }
    ...
    /** Returns a copy of the comparators supported by the TimeTableField.
     */
     ...
     *  @return An instance of TimeFieldDataIterator.
    ...
    public FieldDataIterator iterator(int firstIndex) {
        ...
        for (int i=0; i<dataSize; i++)
            recIndices.add(i);
        return new TimeFieldDataIterator(recIndices, firstIndex, this);
    }
     ...
     *  @see #getComparator(String)
     */
    public boolean compareTo(int recIndex, TimeComparator comparator, CTime value) {
    ...
    /** @see #compareTo(int, TimeComparator, CTime)
     */
    public boolean compareTo(int recIndex, ObjectComparator comparator, Object value) {
        return compareTo(recIndex, (TimeComparator) comparator, (CTime) value);
    }
     ...
     *  @see #sort(ObjectBaseArray, int, int, TimeComparator)
     */
    public void sort(ObjectBaseArray data, TimeComparator comparator) {
     ...
     *  @param comparator The Comparator used to sort the array.
     */
    public void sort(ObjectBaseArray data, int first, int last, TimeComparator comparator) {
...
//        int finish = (base.start()).distance( last );
     ...
     *  @return The new order of the elements of the array. For example the first int in this array contains the index
     *  that the first element in the sorted <code>data</code> array had before sorting.
     *  @see #getOrder(ObjectBaseArray, int, int, TimeComparator)
     */
    public IntBaseArray getOrder(int first, int last, ObjectComparator comparator) {
        ...
        for (int i=0; i<dataSize; i++)
            dataClone.add(get(i));
        return getOrder(dataClone, first, last, (TimeComparator) comparator);
    }
     ...
     *  that the first element in the sorted <code>data</code> array had before sorting.
     */
    public static IntBaseArray getOrder(ObjectBaseArray data, int first, int last, TimeComparator comparator) {
...
//        int finish = (base.start()).distance( last );
    ...
    public void applyState(StorageStructure fieldMap) {
        ...
        data = (CTimeBaseArray) fieldMap.get("Data");
    }
...
class TimeSecondPredicateComparator implements SecondPredicateComparator {
    CTime secondPredicate = null;
    TimeComparator comparator = null;

    public TimeSecondPredicateComparator(TimeComparator comparator, CTime secondPredicate) {
        this.secondPredicate = secondPredicate;
        this.comparator = comparator;
    }

    public boolean execute(CTime firstPredicate) {
...
abstract class TimeComparator implements ObjectComparator {
    public abstract boolean execute(CTime first, CTime second);
    public boolean execute(Object first, Object second) {
        return execute((CTime) first, (CTime) second);
    }
}

class LessTime extends TimeComparator {
...
class LessEqualTime extends TimeComparator {
    public boolean execute(CTime first, CTime second) {
        if (first == null) {
...
class GreaterTime extends TimeComparator {
    public boolean execute(CTime first, CTime second) {
        if (first == null) {
            if (second == null) return false;
            return false;
...
class GreaterEqualTime extends TimeComparator {
    public boolean execute(CTime first, CTime second) {
        if (first == null) {
            if (second == null) return true;
            return false;
...
class EqualTime extends TimeComparator {
    public boolean execute(CTime first, CTime second) {
        if (first == null) {
            if (second == null) return true;
            return false;
...
class NotEqualTime extends TimeComparator {
    public boolean execute(CTime first, CTime second) {
        if (first == null) {
            if (second == null) return false;
            return true;
