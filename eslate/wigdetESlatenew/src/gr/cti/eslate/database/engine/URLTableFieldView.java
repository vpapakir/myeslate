...
/** URLTableFieldView implements a view on a URLTableField, called the base field. A view of a table field is
 ...
 */
// The 'data' IntBaseArray of the URLTableFieldView is always empty.
public class URLTableFieldView extends URLTableField implements TableFieldView {
    public static final int FORMAT_VERSION = 1;
    static final long serialVersionUID = 12;
    /** The field on which this view is based. */
    URLTableField baseField = null;
    ...
    public URLTableFieldView() {
    }

    protected URLTableFieldView(URLTableField field) {
        super(field.getName(), field.isEditable(), field.isRemovable(), field.isHidden());
        baseField = field;
    }

    /** Sets the TableView of the URLTableFieldView */
    void setTable(Table table) {
        ...
        if (baseField == null && baseFieldName != null) {
            try{
                baseField = (URLTableField) tableView.getBaseTable().getTableField(baseFieldName);
            }catch (InvalidFieldNameException exc) {
                System.out.println("InvalidFieldNameException in URLTableFieldView setTable(). Should never occur...");
            }
            baseFieldName = null;
        }
    }
...
//        baseField.remove(recIndex);
    ...
    public ArrayBase getData() {
        int dataSize = size();
        URLBaseArray array = new URLBaseArray(dataSize);
