...
public class ClockPrimitives extends PrimitiveGroup {
    ...
    public final LogoObject pSETCLOCKDATE(InterpEnviron env, LogoObject obj[]) throws LanguageException {
        ...
        if (length > 0) {
            c.set(GregorianCalendar.DAY_OF_MONTH, ((LogoList) obj[0]).pickInPlace(0).toInteger());
        }
        if (length > 1) {
            c.set(GregorianCalendar.MONTH, ((LogoList) obj[0]).pickInPlace(1).toInteger() - 1);
        }
        if (length > 2) {
            c.set(GregorianCalendar.YEAR, ((LogoList) obj[0]).pickInPlace(2).toInteger());
        }
        if (c == null) {
    ...
    public final LogoObject pCLOCKDATE(InterpEnviron env, LogoObject obj[]) throws LanguageException {
        ...
        lo[0] = new LogoWord(calendar.get(GregorianCalendar.DAY_OF_MONTH));
        lo[1] = new LogoWord(calendar.get(GregorianCalendar.MONTH) + 1);
        lo[2] = new LogoWord(calendar.get(GregorianCalendar.YEAR));
        //    lo[3] = new LogoWord(t.usec / 1000);
        return new LogoList(lo);
    }
