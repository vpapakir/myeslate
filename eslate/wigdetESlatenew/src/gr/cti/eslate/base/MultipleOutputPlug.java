...
/**
 * Implements multiple output shared object plugs.
 *
 * @author      Petros Kourouniotis
 * @author      Kriton Kyrimis
 * @version     2.0.21, 28-Sep-2007
 */
public class MultipleOutputPlug extends SharedObjectPlug
{
  /**
   * Constructs a multiple output shared object plug.
  ...
  public MultipleOutputPlug(ESlateHandle handle, ResourceBundle bundle,
                           String key, Color cl, Class shObjClass,
                           SharedObject so)
    throws InvalidPlugParametersException
  {
    super(handle, bundle, key, cl, shObjClass, so, (SharedObjectListener)null);
  }
  ...
  public ImageIcon getIcon()
  {
    if (internals.dependentPlugs.size() == 0) {
      if (icon == null) {
        icon = createPlugIcon(Plug.class.getResource("mo.gif"));
      }
      return icon;
    } else {
      if (icon_o == null) {
        icon_o = createPlugIcon(Plug.class.getResource("mo_o.gif"));
      }
      return icon_o;
    }
  }
