...
public class CanvasRectangle extends CustomCanvasTool{
...
public String getToolName(){return "RECTANGLE";};
...
//System.out.println("rectangle:MouseClicked");
 ...
 if (SwingUtilities.isLeftMouseButton(e)) {
  ...
  if (oddClick) {x0=e.getX(); y0=e.getY();}
   ...
   gx.setPAINT();
   gx.Rectangle(x0,y0,e.getX(),e.getY());
   DidSomeChange();
  }
 }
}
...
//System.out.println("rectangle:MousePressed");
 ...
 gx.setXOR();
 gx.Rectangle(x1,y1,e.getX(),e.getY());
 c.repaint();
}

public void mouseReleased(MouseEvent e){ //MouseListener//
//System.out.println("rectangle:MouseReleased");
 gx.setPAINT();
 gx.Rectangle(x1,y1,e.getX(),e.getY()); //just draw the last XOR line with its final color
 //System.out.println("drawing rectangle !!!");
 DidSomeChange();
}

public void mouseDragged(MouseEvent e){ //MouseMotionListener//
//System.out.println("rectangle:MouseDragged");
 ...
 if (SwingUtilities.isLeftMouseButton(e) && oddClick) //fix for when user drags the second point after a single Left click
  {oddClick=false; gx.setPAINT(); gx.Rectangle(x0,y0,e.getX(),e.getY());}
 else{ //?
  gx.setXOR();
  gx.Rectangle(x1,y1,gx.getX(),gx.getY()); //erase old line (redraw it in XOR mode)
  gx.Rectangle(x1,y1,e.getX(),e.getY()); //draw new line
 }
 c.repaint();
}
