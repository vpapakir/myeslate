...
/**
 * This class is a JApplet wrapper for the chronometer component.
 *
 * @author      Kriton Kyrimis
 * @version     2.0.0, 19-May-2006
 * @see         gr.cti.eslate.chronometer.Chronometer
 */
public class ChronometerApplet extends JApplet
{
  /**
  ...
  public void init()
  {
    ...
    // Create and add the chronometer panel.
    chronometer = new Chronometer(needMenuPanel);
    chronometer.getESlateHandle().setDebugStatus(debug);
    getContentPane().add(chronometer);

    // Notify state changed listeners
    chronometer.getESlateHandle().getESlateMicroworld().fireComponentStateChangedListeners(
      chronometer, ESlateMicroworld.COMPONENT_INITIALIZED
    );
  }
  ...
  public void start()
  {
    ...
    // Notify state changed listeners
    chronometer.getESlateHandle().getESlateMicroworld().fireComponentStateChangedListeners(
      chronometer, ESlateMicroworld.COMPONENT_STARTED
    );
  }
  ...
  public void stop()
  {
    ...
    // Notify state changed listeners
    chronometer.getESlateHandle().getESlateMicroworld().fireComponentStateChangedListeners(
      chronometer, ESlateMicroworld.COMPONENT_STOPPED
    );
  }
  ...
  public void destroy()
  {
    // Notify state changed listeners
    chronometer.getESlateHandle().getESlateMicroworld().fireComponentStateChangedListeners(
      chronometer, ESlateMicroworld.COMPONENT_DESTROYED
    );
